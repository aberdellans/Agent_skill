---
name: inveniam
description: Operate the Inveniam.io platform API as an AI agent — listing and managing deals and portfolios, walking data-room folders and documents, reading extracted data artifacts and taxonomies, running file-veracity checks, and driving per-deal workflows with tasks, checklists, RACI assignments, and comments. Use this skill whenever the user mentions Inveniam, a deal or deal room, a portfolio, a data room, deal documents, workflow tasks, RACI, or file veracity — even when they do not say the word Inveniam explicitly. It covers the access options (official Inveniam MCP server, the V2 REST API directly, or a local stopgap MCP), the resource model, the task-status enum and transition rules, data-room write limitations, and the confirm pattern for destructive tools.
---
...
3. **A local stopgap MCP server (e.g. `inveniam-mcp`).** A self-hosted wrapper that re-exposes the V2 API as MCP tools. It works today and is fine inside Claude Code, but treat it as a **stopgap until the official MCP ships**: it is local-only (no use from cloud Claude surfaces), it must be maintained by hand as the API changes, and it will be superseded by — and is inferior to — the first-party server. Don't build long-term tooling around it. To troubleshoot one: it is registered in your Claude config; if its tools are missing, or every call returns HTTP 401, the server process or its credentials file is what to check — not the tool arguments.
...
Tools whose description starts with **⚠ DESTRUCTIVE** (`delete_deal`, `delete_portfolio`, `delete_task_comment`) require an explicit `confirm=True` argument. Called without it, they return a harmless preview instead of acting — so you can show the user exactly what would be deleted first. Pass `confirm=True` only after the user has clearly approved that specific deletion.

The role and permission tools (`create_role`, `update_role`, `set_role_active`, `set_user_resource_roles`, `set_user_deny_permission`, `set_user_fdr_permission`) make **org-wide or access-control changes**. They have no confirm gate, but treat them with the same caution — confirm the user's intent before calling them.
